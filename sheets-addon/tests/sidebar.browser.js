...
(async () => {
    ...
    window.google = {script: {run: {
      withSuccessHandler(success) {
        return {withFailureHandler() {
          return {
            getSheetContext() { success({spreadsheet:'Prereasoner',activeSheet:'Customers',totalRows:23,fingerprint:'sheet-v1',tables:[{name:'Customers',range:'A1:E24',rows:23,columns:5,preview:[['order ID','customer','city','tier'],['101','Sherlock Holmes','London','Gold'],['102','Sherlock Holmes','London','Gold'],['103','Sherlock Holmes','London','Gold']]}]}); },
            ...
            askPrereasoner(payload) { setTimeout(()=>success({question:payload.question,conversationId:'c_0123456789abcdef0123456789abcdef',history:[],reply:'The total is **US$1,240**. [Open source](https://example.com/source).',analysis:{analysis_id:'a_11111111111111111111111111111111',slug:'france_total',revision:1,display_name:'France total'},reasoning:[{label:'France orders',kind:'filter',detail:'Kept the rows that match the question.',sectionId:'france',sectionLabel:'France orders',sectionQuestion:'Orders in France',sectionInputs:[]},{label:'Total amount',kind:'group_agg',detail:'Grouped the matching rows and calculated the measure.',sectionId:'result',sectionLabel:'Total amount',sectionQuestion:'Total amount in France',sectionInputs:['france'],isOutput:true}],result:{headers:['total_usd'],rows:[['1240']]},context:{spreadsheet:'Prereasoner',activeSheet:'Customers',totalRows:23,fingerprint:'sheet-v1',tables:[{name:'Customers'}]}}), 500); }
          };
        }};
      }
    }}};
  ...
  const fixtureContext = JSON.stringify({spreadsheet:'Prereasoner',activeSheet:'Customers',totalRows:23,fingerprint:'sheet-v1',tables:[{name:'Customers'}],privacyUrl:'https://chat.prereasoner.com/privacy',termsUrl:'https://chat.prereasoner.com/terms',supportUrl:'https://chat.prereasoner.com/support'});
