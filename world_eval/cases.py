...
_CITY_COUNTRY = ('JOIN LATERAL (SELECT country FROM knowledgebase."Cities" wc WHERE lower(wc.name)=lower({c}.city) '
...
CASES = [
    {"label": "sales_by_continent", "cap": "country->continent | group+sum", "tables": [SALES],
     ...
     "oracle": 'SELECT w.continent, SUM(s.amount) FROM sales s '
               'JOIN knowledgebase."Countries" w ON lower(s.country)=lower(w.name) GROUP BY w.continent'},

    {"label": "top_continent", "cap": "country->continent | argmax", "tables": [SALES],
     "question": "which continent has the highest total amount",
     "oracle": 'SELECT w.continent FROM sales s JOIN knowledgebase."Countries" w ON lower(s.country)=lower(w.name) '
    ...
    {"label": "total_in_asia", "cap": "country->continent | filter+sum (scalar)", "tables": [SALES],
     ...
     "oracle": "SELECT SUM(s.amount) FROM sales s JOIN knowledgebase.\"Countries\" w ON lower(s.country)=lower(w.name) "
    ...
    {"label": "countries_in_europe", "cap": "country->continent | filter+count (scalar)", "tables": [SALES],
     ...
     "oracle": "SELECT count(*) FROM sales s JOIN knowledgebase.\"Countries\" w ON lower(s.country)=lower(w.name) "
    ...
    {"label": "amount_by_currency", "cap": "country->currency | group+sum", "tables": [SALES],
     ...
     "oracle": 'SELECT w.currency, SUM(s.amount) FROM sales s '
               'JOIN knowledgebase."Countries" w ON lower(s.country)=lower(w.name) GROUP BY w.currency'},

    {"label": "avg_atomic_mass", "cap": "element->mass | avg (scalar)", "tables": [SAMPLES],
     "question": "average atomic mass",
     "oracle": 'SELECT AVG(w.mass) FROM samples s JOIN knowledgebase."Elements" w ON lower(s.element)=lower(w.name)'},
