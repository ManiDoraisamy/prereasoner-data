...
|---|---|---|
| `vector` (pgvector) | `world."words".embedding vector(384)` + HNSW `<=>` cosine search; per-user `"<t> unconnected to wikipedia".embedding vector(896)` | `runtime16/scripts/setup_words16.py` DDL; `query16._nn` / `_cell_bridge_sql` (LATERAL `<=>`); `world17._persist_main_unconn` (`vector({self.hdim})`, hdim=896 for the unified Qwen encoder) |
...
- `world` — serving schema: `"words"`, `"types"`, friendly tables + views.
- `wikipedia` — qid-keyed faithful tables, exact-Wikidata-label names
  (`build_wikipedia_schema.py`, `sync_wikidata_world.ensure_table`).
- `"<google-sub>"` — per-user, created at request time by
  `query14._load_user_schema` (`CREATE SCHEMA IF NOT EXISTS "<sub>"`); `<sub>` is
  the server-verified Firebase/Google `sub` (long numeric id). Queries run with
  `SET search_path TO "<sub>", wikipedia, world, public` (wikipedia first so bare
...
### world."words" — the resolution index
...
`type=<world."types".label>`, e.g. `academic journal` — note: NOT snake_cased).

### world."types" — taxonomy DAG
...
|---|---|---|
| `wikipedia."<exact Wikidata label>"` | e.g. `wikipedia."city"`, `wikipedia."hospital"`, `wikipedia."academic journal"` — label from `world."types".label`, truncated to 63 chars; shared labels suffixed `" (Qxxx)"` by build_wikipedia | `sync_wikidata_world.ensure_table` (via `ensure_entity`/`lazy_resolve`, called from `query16._city_bridge_sql`, `world17._resolve_world_qid`/`_serve_world_type`); pre-creatable by `mirror_world_schema.py` (world.* mirror) + `build_wikipedia_schema.py` (qid-PK copies) |
| rows in wikipedia tables | one row per entity, qid PK, all-TEXT property columns, item-valued props store the related entity's **qid** (FK) | `ensure_entity` → `fetch_one` (WDQS), `ON CONFLICT (qid) DO NOTHING`; each also INSERTs a `world."words"` row |
...
Pre-sync required: `world."words"` (all resolution paths gate on it — an empty
index resolves nothing, and even `_resolve_world_qid`'s lazy branch fires only
after words/types lookups), `world."types"` (lazy table naming reads
`types.label`), `public.settlement` (NEARBY reads it directly),
`world."Cities"/"Countries"/…` (planner + world18 read them).

## 6. Connection contract

Old code: `host = env WORLD_PG_HOST default 34.123.19.176; dbname="world";
user="postgres"; password = env WORLD_PG_PASSWORD (required); TCP ⇒ port 5432 +
sslmode=require; host starting "/" ⇒ Cloud SQL unix socket (no port/ssl)`.
New `db/sync/_conn.py`: all five from `WORLD_PG_HOST/PORT/DB/USER/PASSWORD`,
`WORLD_PG_SSLMODE` default `prefer` (works on both no-SSL docker and Cloud SQL),
...
2. **`world."types"` row count drift** — unify_words_qid's docstring says 202
...
3. **`words.props` shape** — `row_to_json` of whatever the source table looked
   ...
   `world."Cities"` that carried those columns; the merged `build_words.py`
...
7. **is_primary type mismatch** — `world."Cities".is_primary` is `int` (0/1)
   while `world."words".is_primary` is `boolean`; both faithful to source
