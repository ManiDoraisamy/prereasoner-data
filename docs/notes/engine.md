...
|---|---|---|
| server18.py + server17world.py + server19.py | `server.py` | ONE ThreadingHTTPServer. `POST /api/reason`, `POST /api/world` (Firebase auth + RTDB streaming, shared WorldReasoner + shared WORLD_LOCK), `POST /api/dimension` (stateless, own DIM_LOCK), `GET /healthz`. NO legacy route aliases. CORS + MAX_BODY(10MB)/MAX_SHEETS(8)/MAX_ROWS(5000) preserved exactly. |
...
| query12.py | `world_tables.py` | `Query12` → `WorldTableQuery` (implicit world word_* tables, meaning graph, freshness). |
...
| world17.py | `world_query.py` | `Query17World` → `WorldQuery`. `WORLD_MODEL_ROUTE` read via config. |
| world18.py | `world_compose.py` | `Query18World` → `ComposedWorldQuery`. |
| world19.py | `world.py` | `Query19World` → `WorldReasoner` (geo NEARBY wrapper). |
...
| scripts/sync_wikidata_world.py (runtime part) + build_world_wdqs.py (client part) | `world_sync.py` | NEW extraction: `lazy_resolve`, `ensure_entity`, `find_entity`, `discover`, `fetch_one`, `wlabel`, `ensure_table` + the WDQS client (`wdqs`, `V`, `qid_of`, `wbsearch`, `ask`, `ENDPOINT`, `UA`). These are genuinely reachable at serving time (lazy Wikidata fill in entities.py / world_query.py). The bulk-sync CLI (`fetch`, `main`) did not move — that lives in `db/sync`. |
...
renames cannot break loading. Renamed: `Query19World→WorldReasoner`, `Query19→DimensionModel`,
`Query18World→ComposedWorldQuery`, `Query17World→WorldQuery`, `Query17→EncoderQuery`, `Query16→EntityQuery`,
`Query15→RoutedQuery`, `Query14→PgQuery`, `Query12→WorldTableQuery`, `Query11→TableQuery`,
...
| `PORT` | `8080` | HTTP port |
| `WORLD_PG_HOST` | `localhost` | Postgres host, or a unix-socket dir path (Cloud Run) |
| `WORLD_PG_PORT` | `5432` | |
| `WORLD_PG_DB` | `world` | |
| `WORLD_PG_USER` | `postgres` | |
| `WORLD_PG_PASSWORD` | *(none)* | REQUIRED at connect time; clear RuntimeError if missing |
| `WORLD_PG_SSLMODE` | `prefer` | NEW: source hardcoded `require` (Cloud SQL public IP); `prefer` also works with local docker Postgres. Set `require` in production. |
...
| `WORLD_MODEL_ROUTE` | `1` | `0` disables model-driven column routing (value-membership fallback). Unchanged name (no version). |
...
| legacy routes `/infer-runtime20*`, `/query`, `/infer` | intentionally dead; the web frontend already calls `/api/reason` / `/api/world`. |
...
| `words.db` | referenced only by the SQLite fallback path (`WorldTableQuery._connect`/`ambiguities`/`_world_rows`), and `PgQuery` overrides all three on every live route (verified against the MRO), so the 34 MB file is dead weight. Existed in the source data dir; not shipped. |
| `train11.py`, `train17.py`, `scripts/*` (except the constants/functions extracted into `taxonomy.py`/`world_sync.py`), `build_world_pg.py`, `build_world_wdqs.py` (bulk part), `route_model_test.py` | training / offline DB-build / experiment scripts — live in `training/` and `db/sync` per their own migration notes. |
...
2. `EncoderQuery()` standalone now loads the shipped unified model (`encoder_meta.pt`/`encoder.pt`) instead
   of the retired runtime17 artifacts. Serving paths (`WorldQuery`, `DimensionModel`) are byte-for-byte the
   same loads as before.
3. `_pg()` sslmode is `prefer` by default (was hardcoded `require`); set `WORLD_PG_SSLMODE=require` for
...
6. `/api/reason` and `/api/world` share ONE `WorldReasoner` and ONE lock (in the source they were separate
...
- `world."Country Aliases"`, `world."types"`, `public.settlement`, `wikipedia.*` etc. must exist in the DB
