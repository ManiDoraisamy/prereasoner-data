...
| SQLite-backed evaluation could label timestamp text as a date and fail while loading an unrelated mapped column | Generated mappings use native PostgreSQL `DATE`, preserve SQLite date/timestamp text, and compare typed date literals against its ISO representation without losing time data |
