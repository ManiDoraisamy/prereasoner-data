...
def test_excel_restore_does_not_bind_a_google_sheet_by_matching_contents():
    ...
    connection = _Connection(cursor)
    with patch.object(sheet_sessions, "_pg", return_value=connection):
        restored = sheet_sessions.restore_sheet_session("user", "excel_abcdefghij", tables, host="excel")
    assert restored["conversation_id"] is None and restored["legacy"] is False
    assert restored["host"] == "excel"
...
def test_save_requires_ownership_and_persists_a_bounded_snapshot():
    ...
    assert json.loads(upsert[1][4]) == state
    assert upsert[1][5] == len(upsert[1][4].encode("utf-8"))
