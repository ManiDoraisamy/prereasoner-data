...
**An interpretable model for questions over your own data.** PreReasoner types your question and
your columns against *named dimensions*, assembles the exact SQL those dimensions imply, and hands
back the derivation as the answer. There is no decoder in the number path: nothing is sampled, so
nothing can be fabricated. The same input yields the same dimensions, the same query, and the same
result, which means a past answer can be replayed and checked rather than taken on trust.

This is not an explainer bolted onto a black box. A generated explanation is a second guess that may
not describe the computation it claims to; a derivation *is* the computation. And when the data does
not support an answer, the engine says so instead of returning a plausible number.

## Structured retrieval, not vector similarity

Ask *"total sales in France"* of a sheet whose only location column holds `Paris` and `Strasbourg`:

| Approach | What happens |
|---|---|
| Text-to-SQL | Cannot write the query at all — there is no country column to filter on. |
| Vector RAG | Embeds the cities and retrieves what *reads* as French. Strasbourg returns, and so does Kehl, a German town 2 km away. Similarity is not membership. |
| **PreReasoner** | Types the column as an entity reference, resolves each city to its Wikidata identity, and joins to the country a publisher asserts it belongs to. Kehl is in Germany, so it is not in the sum — and the join that decided it is visible. |

Text columns are treated as typed foreign keys into real reference tables, so a question becomes an
exact relational join instead of a nearest-neighbour guess. Three kinds of data compose in one plan:
...
The answer is computed by SQL, not generated by a decoder. Inference is deterministic for fixed
inputs, configuration, database state, and model artifacts. The frozen Qwen model is used as an
encoder for intent and schema signals; it does not call `generate()` to write SQL or numbers.
...
are never treated as facts memorized by the model. `knowledgebase.schedule` records what each world
table promises and when it last kept that promise, so staleness is reported rather than assumed.

Weights are open and published; the engine runs on CPU and self-hosts, so regulated data never has
to leave your network. What determinism does *not* buy you is stated plainly in
[Accuracy Boundary](#accuracy-boundary).
