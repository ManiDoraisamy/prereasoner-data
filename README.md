...
Text-to-SQL cannot write the query. There is no country column to filter on.

Vector RAG embeds the cities and returns what reads as French. Strasbourg comes back. So does Kehl,
a German town 2 km from Strasbourg. Similarity is not membership.

PreReasoner links text columns to public and private reference tables with foreign keys. It resolves
each city to its Wikidata entity and joins to the country that entity belongs to. Kehl is in Germany,
so it stays out of the total, and you can see the join that decided it.

Three kinds of data compose in one query:
...
table is expected to refresh and when it last did, so stale data is reported instead of assumed
current.
