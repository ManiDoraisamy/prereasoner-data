"""The RETRIEVAL embedder for entity resolution (separate from the interpretable unit encoder).

Why a different model: a generative LM's mean-pooled hidden states are an anisotropic space that is NOT a
metric space for entity-linking (measured: "USA" is nearer to "Australia" than to "United States").
bge-small-en-v1.5 is contrastively trained for retrieval, so cosine distance is meaningful: "US"/"USA"/"UK"/
"Deutschland"/"Holland"/"Bombay" all land on the right canonical entity, and prepositions ("in") /
non-entities ("Indiana") stay below threshold. This embedder backs:
  - the world `words` vector index (one row per canonical entity)
  - CSV cell-value resolution + spaCy-parsed prompt-word resolution (engine.entities)

bge-small sentence embedding = the [CLS] token of last_hidden_state, L2-normalized (per the model card), so
cosine similarity == dot product. 384-dim. CPU-fine (33M params).
"""
