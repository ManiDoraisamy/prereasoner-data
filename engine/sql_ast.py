"""Typed, immutable SQL AST for deterministic SQL search — the IR.
...
scope-aware and always runs before a tree is lowered to a target.

This module owns the node grammar, typing, and validation, and nothing else. Lowering the
validated tree to an executable target lives in `deterministic/emitter/` — SQL text under
`emitter/sql/`, Python objects under `emitter/py/`. This module must never import from
...
def _validate_query(query: Query, outer_scope: frozenset[str]) -> None:
    ...
    for item in query.select:
        validate_expression(item.expression, visible)
    for term in query.order_by:
        validate_expression(term.expression, visible)
    ...
    referenced = set()
    for item in query.select:
        referenced.update(expression_tables(item.expression))
    referenced.update(column.table for column in query.group_by)
    for term in query.order_by:
        referenced.update(expression_tables(term.expression))
...
def validate_expression(expr: ScalarExpr, visible: frozenset[str]) -> None:
    ...
    if isinstance(expr, Aggregate):
        ...
        if isinstance(expr.operand, BinaryExpr):
            ...
            validate_expression(expr.operand, visible)               # e.g. SUM(amount * rate)
        return
    if isinstance(expr, BinaryExpr):
...
def _validate_predicate(predicate: Predicate | None, visible: frozenset[str]) -> None:
    ...
    if isinstance(predicate, InPredicate):
        validate_expression(predicate.left, visible)
    ...
    validate_expression(predicate.left, visible)
    validate_expression(predicate.right, visible)
...
def expression_tables(expr: ScalarExpr) -> set[str]:
    if isinstance(expr, ColumnRef):
        return {expr.table}
    if isinstance(expr, Aggregate):
        return expression_tables(expr.operand)
    if isinstance(expr, BinaryExpr):
        return expression_tables(expr.left) | expression_tables(expr.right)
    return set()
...
def _predicate_tables(predicate: Predicate | None) -> set[str]:
    ...
    if isinstance(predicate, ExistsPredicate):
        return set()
    if isinstance(predicate, InPredicate):
        out = expression_tables(predicate.left)
        if isinstance(predicate.source, tuple):
            for expression in predicate.source:
                out.update(expression_tables(expression))
        return out
    return expression_tables(predicate.left) | expression_tables(predicate.right)
