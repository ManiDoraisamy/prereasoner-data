...
WHAT IT DOES:
  ...
  2. One EMPTY table per router leaf, named by its EXACT Wikidata label, with the faithful Wikidata property columns
     (copied from the world."<leaf>" mirror schema that was built by property-frequency discovery) + **qid as PRIMARY KEY
...
KEEP_SCHEMAS = {"public", "world", "wikipedia", "information_schema", "pg_catalog", "pg_toast"}
...
def main():
    ...
    cur.execute("CREATE SCHEMA IF NOT EXISTS wikipedia")
    cur.execute('SELECT qid, label FROM world."types"')
    ...
    for leaf in sorted(LEAF_QID):
        ...
        if not _exists(cur, "world", leaf):                                 # no faithful schema mirror -> discover later
            skipped += 1; continue
        cur.execute("SELECT 1 FROM information_schema.columns WHERE table_schema='world' AND table_name=%s "
        ...
        seen[label] = leaf
        cur.execute(f'DROP TABLE IF EXISTS wikipedia."{label}" CASCADE')    # fresh + idempotent (tables are empty)
        cur.execute(f'CREATE TABLE wikipedia."{label}" (LIKE world."{leaf}")')   # faithful columns
        cur.execute(f'ALTER TABLE wikipedia."{label}" ALTER COLUMN qid SET NOT NULL')
        cur.execute(f'ALTER TABLE wikipedia."{label}" ADD PRIMARY KEY (qid)')     # qid PK — joins are qid PK/FK
        created += 1
    ...
    if not args.yes:
        ...
        cur.execute("SELECT count(*) FROM information_schema.tables WHERE table_schema='wikipedia'")
