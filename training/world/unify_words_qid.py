...
instance ('city' ALWAYS means Q515). Denormalizing it onto 213k word rows means rewriting every row — and knowledgebase."words"
has an HNSW index on a vector(384), so each rewrite re-indexes its embedding (slow + heavy bloat) on the LIVE table.
The normalized + online-safe equivalent: store the legacy resolver string on the ~5 TYPE NODES (knowledgebase."types" is 202
...
def main():
    if not os.environ.get("KB_PG_PASSWORD"):
        print("set KB_PG_PASSWORD (Secret Manager) to run the live migration"); return 1
    ...
    cur.execute('SELECT qid, label FROM knowledgebase."types" WHERE qid = ANY(%s)', (qids,))
    ...
    if not _ddl_with_retry(conn, 'ALTER TABLE knowledgebase."types" ADD COLUMN IF NOT EXISTS "resolver_type" TEXT', "ALTER types"):
        print("ABORT: could not alter knowledgebase.\"types\""); return 1
    for s, q in RESOLVER_QID.items():
        cur.execute('UPDATE knowledgebase."types" SET "resolver_type"=%s WHERE qid=%s AND "resolver_type" IS DISTINCT FROM %s', (s, q, s))
    cur.execute('CREATE INDEX IF NOT EXISTS ix_types_resolver ON knowledgebase."types"("resolver_type")')
    cur.execute('SELECT "resolver_type", qid, label FROM knowledgebase."types" WHERE "resolver_type" IS NOT NULL ORDER BY 1')
    ...
    cur.execute("SELECT 1 FROM information_schema.columns WHERE table_schema='knowledgebase' AND table_name='words' AND column_name='type_qid'")
    if cur.fetchone():
        _ddl_with_retry(conn, 'ALTER TABLE knowledgebase."words" DROP COLUMN IF EXISTS "type_qid"', "DROP words.type_qid")

    # 3) VERIFY the qid walk Paris (a city INSTANCE) -> city node -> ... -> root, via the type-node map
    cur.execute('''SELECT w.canonical, t.qid, t.label FROM knowledgebase."words" w
                   JOIN knowledgebase."types" t ON t."resolver_type" = w.type
    ...
    for _ in range(20):
        cur.execute('SELECT parent_qid FROM knowledgebase."types" WHERE qid=%s', (q,))
        r = cur.fetchone()
        if not r or not r[0] or r[0] == q:
            break
        cur.execute('SELECT label FROM knowledgebase."types" WHERE qid=%s', (r[0],))
        lb = cur.fetchone(); path.append(lb[0] if lb else r[0]); q = r[0]
    print("\nqid walk:", " -> ".join(path))

    # coverage: how many instances now reach a taxonomy node by qid
    cur.execute('''SELECT COUNT(*) FILTER (WHERE t.qid IS NOT NULL), COUNT(*) FROM knowledgebase."words" w
                   LEFT JOIN knowledgebase."types" t ON t."resolver_type"=w.type''')
    linked, total = cur.fetchone()
    print(f"instances linked into the qid hierarchy: {linked}/{total} ({linked/total:.1%})")
    cur.execute('''SELECT w.type, COUNT(*) FROM knowledgebase."words" w LEFT JOIN knowledgebase."types" t ON t."resolver_type"=w.type
