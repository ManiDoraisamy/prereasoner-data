"""
gen11 edges — gen10's 9 edge types + a 10th `fk` edge: a foreign-key column's units linked to the
referenced primary-key column's units ACROSS tables (the cross-table relational-attention edge). The first 9
indices are unchanged so a gen10 checkpoint warm-starts directly (the 10th eb row inits to zero).

A unit may carry `ref=(ref_table, ref_col)` (set on a FK column from relate11's discovery); edges wires
E_FK between that column's units and the referenced PK column's units. Within a table the gen7/10
parse-tree edges apply (same_col/same_row/same_cell); q/sql edges as in gen10.
"""
    ...
    for i in range(S):
        for j in range(S):
            ...
            if g[i] == "schema" and g[j] == "schema":
                if tb[i] == tb[j]:                                          # within a table: gen7 parse-tree edges
