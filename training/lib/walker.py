"""
gen7 walker — consumes the pre-built UNIT list (build7.py; spaCy/ACE already ran there) and produces the
parse-tree edges + per-unit target/mask. Edges: SAME_CELL links a cell's head to its content-noun units;
SAME_COL links a column name to its cells + values down a column (where a name AGGREGATES its values);
SAME_ROW links values across a row. No spaCy/ACE here — just structure + the stored targets.
"""
