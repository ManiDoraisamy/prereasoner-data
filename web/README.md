...
| Page | Purpose |
|---|---|
| `index.html` | Attach CSV/Excel/Google Sheets data and begin a question — served at `/` and at the single-source landings `/sheet`, `/excel`, `/csv` (Hosting rewrites; the add button narrows to that source) |
...
| `sheets.html` | Google Sheets picker/import flow at `/sheets` (returns to the route that opened it) |
